...
export function parseMarker(content) {
    ...
    if (!fmMatch)
        return { sync: false, target: "discard" };
    const fm = fmMatch[1];
    const syncMatch = fm.match(/^sync:\s*(true|false)/m);
    if (!syncMatch || syncMatch[1] !== "true")
        return { sync: false, target: "discard" };
    const targetMatch = fm.match(/^target:\s*(memory|core-file|branch)/m);
    const target = targetMatch ? targetMatch[1] : "discard";
    return { sync: true, target, description: fm.match(/^description:\s*(.+)/m)?.[1] };
}
export function classifyArtifacts(artifacts) {
    const plan = { memory: [], coreFiles: [], branch: [], discard: [] };
    for (const a of artifacts) {
        const marker = parseMarker(a.content);
        const classified = { path: a.path, marker, content: a.content };
        if (marker.sync) {
            if (marker.target === "memory")
                plan.memory.push(classified);
            else if (marker.target === "core-file")
                plan.coreFiles.push(classified);
            else if (marker.target === "branch")
                plan.branch.push(classified);
        }
        else {
...
export function applyMemorySync(plan, repoRoot) {
...
export function renderSyncPlan(plan) {
    ...
    if (plan.memory.length)
        out += `**Memory (${plan.memory.length}):**\n` + plan.memory.map(a => `  - ${a.path}`).join("\n") + "\n\n";
    if (plan.coreFiles.length)
        out += `**Core Files (${plan.coreFiles.length}):**\n` + plan.coreFiles.map(a => `  - ${a.path}`).join("\n") + "\n\n";
    if (plan.branch.length)
        out += `**Branch (${plan.branch.length}):**\n` + plan.branch.map(a => `  - ${a.path}`).join("\n") + "\n\n";
    if (plan.discard.length)
        out += `**Discard (${plan.discard.length}):**\n` + plan.discard.map(a => `  - ${a.path}`).join("\n") + "\n\n";
    return out;
}
