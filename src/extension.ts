...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      const cfg = getMode(params.mode as string);
      if (!cfg) return { content: [{ type: "text", text: `Unknown mode: ${params.mode}` }] };

      const requestId = randomUUID();
      const req = buildSpawnRequest(cfg, params.task as string, requestId);
      const result = await emitSpawn(pi, req);
